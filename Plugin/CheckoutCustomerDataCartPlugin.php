...
class CheckoutCustomerDataCartPlugin
{
    /**
    ...
    {
        $result['subtotal_incl_tax_html'] = $this->checkoutHelper->formatPrice($this->getSubtotalInclTax());
        $result['subtotal_excl_tax_html'] = $this->checkoutHelper->formatPrice($this->getSubtotalExclTax());
        ...
        if (is_array($result['items'])) {
            foreach ($result['items'] as $key => $itemAsArray) {
                if ($item = $this->findItemById($itemAsArray['item_id'], $items)) {
                    ...
                    $result['items'][$key]['product_price_html']=$this->itemPriceRenderer->toHtml();
                }
            }
        }
        return $result;
